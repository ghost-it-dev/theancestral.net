...
function Page() {
	...
	const { register, handleSubmit, control, reset, formState: { errors } } = useForm<PostCreateData>({ resolver: zodResolver(postCreateSchema) });
	const { field: descriptionField } = useController({ name: 'description', control, defaultValue: '' });
	const { field: publicPostField } = useController({ name: 'publicPost', control, defaultValue: false });
			...
			<div className='border-b border-t border-[#1F2C37] py-4 pb-4 px-4 xl:border-t-0 xl:pt-6 h-[105px] flex items-center'>
				<div className='flex flex-col flex-1'>
					...
					{publicPostField.value ?
						<div onClick={() => publicPostField.onChange(!publicPostField.value)} className='flex items-center gap-1 cursor-pointer'>
