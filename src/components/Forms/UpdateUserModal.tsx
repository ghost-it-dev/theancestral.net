			...
			<form onSubmit={handleUpdateUser} className='mt-2 flex flex-col gap-2' autoComplete='off'>
				...
				<Label label='Role'>
					<div className='flex w-full justify-between gap-2'>
						<div onClick={() => roleField.onChange('user')} className={classNames(roleField.value === 'user' ? 'border-indigo-600' : 'border-[#364150]', 'w-full bg-transparent border-2 px-2 py-1 rounded-md text-center')}>
							<span className='text-lg font-bold'>User</span>
						</div>
						<div onClick={() => roleField.onChange('admin')} className={classNames(roleField.value === 'admin' ? 'border-indigo-600' : 'border-[#364150]', 'w-full bg-transparent border-2 px-2 py-1 rounded-md text-center')}>
